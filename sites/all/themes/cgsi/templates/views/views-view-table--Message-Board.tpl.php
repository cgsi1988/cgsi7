...
<table class="<?php print $class; ?>">
  ...
  <thead>
    <tr>
      ...
      <th class="views-field"><?php print $header['field_msg_bd_post_author_value']; ?></th>
    </tr>
  </thead>
  <tbody>
    <?php foreach ($rows as $count => $row): ?>
      <?php if ($row['nid'] != $previous_row_nid): // If this row contains the same node as the last row we only care about the attached comment ?>
      <tr class="<?php print implode(' ', $row_classes[$count]); ?>">
        <td><?php print $row['title']; ?></td>
        <td><?php print $row['created']; ?></td>
        <td><?php print $row['field_msg_bd_post_author_value']; ?></td>
      </tr>
      <?php endif; ?>
