...
function cgsi_preprocess_page(&$variables, $hook) {		
	...
	if ($variables['breadcrumb'] || $variables['title_content']) {
...
function cgsi_preprocess_views_view(&$vars) {
	if ($vars['view']->name == 'Surnames' && $vars['view']->current_display == 'page_3') cgsi_preprocess_views_view__Surnames__page_3($vars);
}
...
function cgsi_preprocess_views_view_table(&$vars) {
	...
	if ($vars['header']['field_public_lib_searchable_value'] && !$_GET['sort']) {
		$vars['header']['field_public_lib_searchable_value'] = str_replace('sort=asc', 'sort=desc', $vars['header']['field_public_lib_searchable_value']);
	}
}
...
function cgsi_preprocess_views_view__Surnames__page_2(&$vars) {
	// Check to see if any of the exposed filters are in use.
	$filters = $vars['view']->filter;
	$all_empty = TRUE;
	foreach ($filters as $field => $value) {
		// The 'All' check here is for select fields as <Any> returns all, though
		// for our purposes <Any> is the same as none.
		if (isset($vars['view']->exposed_input[$field]) && !empty($vars['view']->exposed_input[$field]) && $vars['view']->exposed_input[$field] != 'All') {
			$all_empty = FALSE;
			break;
		}
	}

	// Do not display the empty text if they are seeing an empty page because
	// they have not yet entered any search terms.
	$vars['empty'] = $all_empty ? '' : $vars['empty'];
